...
export function GainPercent({
  ...
  const successColor = invertColor ? "text-loss" : "text-gain";
  const destructiveColor = invertColor ? "text-gain" : "text-loss";
  const successBg = invertColor ? "bg-loss/10" : "bg-gain/10";
  const destructiveBg = invertColor ? "bg-gain/10" : "bg-loss/10";
  return (
    <div
      className={cn(
        "amount inline-flex items-center justify-end text-right text-sm",
        displayValue > 0 ? successColor : displayValue < 0 ? destructiveColor : "text-flat",
        variant === "badge" && [
          "rounded-md py-px pl-[9px] pr-[12px] font-light",
          displayValue > 0 ? successBg : displayValue < 0 ? destructiveBg : "bg-flat/10",
        ],
        className,
      )}
      {...props}
